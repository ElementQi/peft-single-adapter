...
class DeltaLayer(BaseTunerLayer):
    ...
    def spawn_delta_matrix(self, adapter_name):
        ...
        if use_bias is False:
            ...
            self.delta_theta[adapter_name] = nn.Linear(self.in_features, self.out_features, bias=False).to(device, dtype=torch.bfloat16)
            ...
            self.delta_theta[adapter_name] = nn.Linear(self.in_features, self.out_features, bias=True).to(device, dtype=torch.bfloat16)
            nn.init.zeros_(self.delta_theta[adapter_name].weight)
