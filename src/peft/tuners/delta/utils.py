...
def pack_sign(tensor: torch.tensor) -> tuple[torch.tensor, torch.tensor]:
    ...
    values_tensor = torch.where(
        values_tensor == 1,
        torch.tensor(1, device=values_tensor.device, dtype=torch.uint8),
        torch.tensor(0, device=values_tensor.device, dtype=torch.uint8),
    )
    ...
    padded_values = torch.cat(
        (flat_values, torch.zeros(padded_size - num_values, device=values_tensor.device, dtype=torch.uint8))
    )
    ...
    packed_tensor = torch.sum(
        reshaped_values * (2 ** torch.arange(8, device=values_tensor.device, dtype=torch.uint8)),
        dim=1,
        dtype=torch.uint8,
    )
...
def unpack_and_restore(packed_tensor, shape_tensor):
    ...
    unpacked_values = torch.where(
        unpacked_bits == 1,
        torch.tensor(1, device=packed_tensor.device, dtype=original_dtype),
        torch.tensor(-1, device=packed_tensor.device, dtype=original_dtype),
    )
