...
def del_delta_create_lion_like_column(module, active_block, prefix='', root_module=None):
    """
    Recursively traverse all submodules of a model and apply a specific operation
    to the innermost layers, passing the full name of the layer.
    """
    ...
    for name, layer in module.named_children():
        has_children = True
        new_prefix = f"{prefix}.{name}" if prefix else name
        del_delta_create_lion_like_column(layer, active_block, new_prefix, root_module)
    ...
    if not has_children:
        ...
        if inner_module == "gamma":
            ...
            # init that prefix
            if any(p in prefix for p in active_block):
                the_layer.del_delta_create_column_gamma_sign("default")


def del_delta_create_lion_like_row(module, active_block, prefix='', root_module=None):
    """
    Recursively traverse all submodules of a model and apply a specific operation
    to the innermost layers, passing the full name of the layer.
    """
    ...
    for name, layer in module.named_children():
        has_children = True
        new_prefix = f"{prefix}.{name}" if prefix else name
        del_delta_create_lion_like_row(layer, active_block, new_prefix, root_module)
    ...
    if not has_children:
        ...
        if inner_module == "gamma":
            ...
            # init that prefix
            if any(p in prefix for p in active_block):
                the_layer.del_delta_create_row_gamma_sign("default")
